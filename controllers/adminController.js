...
exports.postAddProduct = (req, res, next) => {
	...
	if (errors.isEmpty()) {
		...
		return product
			.save()
			.then((result) => {
				return Catagory.findByIdAndUpdate(result.catagoryId, {
					$push: { products: { productId: result._id } },
				}).then(() => {
...
exports.getEditProduct = (req, res, next) => {
	...
	Product.findById(productId)
		.populate("catagoryId", "title userId -_id")
		.then((product) => {
...
exports.postEditProduct = (req, res, next) => {
	...
	if (errors.isEmpty()) {
		return Product.findById(req.params.prodId)
			...
			.then((product) => {
				if (!bool) {
					return Catagory.findByIdAndUpdate(product.catagoryId, {
						$push: { products: { productId: product._id } },
					}).then(() => {
						return Catagory.findByIdAndUpdate(oldProductCatagory, {
...
exports.deleteProduct = (req, res, next) => {
	...
	Product.findByIdAndRemove(_id)
		.then(() => {
			return Catagory.findByIdAndUpdate(catagoryId, {
...
exports.getManageProducts = (req, res, next) => {
	Order.find({ "products.product.userId": req.user._id }).then((orders) => {
...
exports.getEditManageProducts = (req, res, next) => {
	Order.find({ "products.product.userId": req.user._id }).then((orders) => {
		res.render("admin/edit-manage-products", {
			title: "manage products",
			path: "/manage-products",
			orders,
		});
